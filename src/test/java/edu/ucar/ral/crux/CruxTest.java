...
public class CruxTest {
  ...
  public void testCruxLocalMultQuestionFiles() throws Exception{
    ...
    Assert.assertEquals( "Incorrect # of validated files", 2, crux.validate( null, null, "src/test/resources/shiporder-pass?.xml" ) );
  }
  ...
  public void testCruxLocalMultStarFiles() throws Exception{
    Crux crux = new Crux();
    //should match shiporder-pass1.xml and shiporder-pass2.xml
    Assert.assertEquals( "Incorrect # of validated files", 2, crux.validate( null, null, "src/test/resources/shiporder-p*.xml" ) );
  }
  ...
  public void testCruxOfflineSchemaFail() throws Exception{
    Crux crux = new Crux();
    try {
      crux.validate( null, null, "src/test/resources/remote-schema.xml" );
    }
    catch( ValidationException e ){
      Assert.assertEquals( "Incorrect # of validation failures", 2, e.getValidationErrors().size() );
  ...
  @Ignore( "Test should be run as needed since it makes outbound network connections and is quite slow" )
  @Test
  public void testCruxOnlineSchema() throws Exception{
    Crux crux = new Crux();
    crux.setAllowingRemoteResources( true );
    crux.validate( null, null, "src/test/resources/remote-schema.xml" );
  }
}